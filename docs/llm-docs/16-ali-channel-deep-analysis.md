...
**图片模型**（通过 `model_setting.IsSyncImageModel()` 配置判断同步/异步）：
```
...
只需更新 `SyncImageModels` 配置（如果需要同步模式），在管理后台添加新模型名即可。
```go
...
func (a *TaskAdaptor) convertToAliRequest(info *relaycommon.RelayInfo, req relaycommon.TaskSubmitReq) (*AliVideoRequest, error) {
    ...
    // 需要确认：wan2.7 系列是否使用相同的 /api/v1/services/aigc/video-generation/video-synthesis 端点
}
```
...
| 文件 | 修改内容 | 前提条件 |
|------|----------|----------|
| `relay/channel/ali/adaptor.go` | `SyncImageModels` 配置更新 | 新图片模型使用同步模式 |
